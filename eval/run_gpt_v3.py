"""批量抽帧并使用 GPT 评测 videos/v3 下的三类视频。"""
...
SAVE_PATH = PROJECT_ROOT / "results" / "gpt-5.6-sol-v3.jsonl"
TEST_SAVE_PATH = PROJECT_ROOT / "results" / "gpt-5.6-sol-v3-test.jsonl"
FRAMES_ROOT = PROJECT_ROOT / "gpt_frames" / "v3"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="批量使用 GPT 评测 videos/v3 下三类 rot/trans 视频"
    )
    parser.add_argument(
        "--videos-root",
...
def default_output_path(videos_root: Path, test_mode: bool) -> Path:
    ...
    return PROJECT_ROOT / "results" / f"{MODEL}-{slug}-v3{suffix}.jsonl"
