...
OUTPUT = f"/data/dataset/TempCompass/case_analysis/NSA-videos/demo_00{x}_a.mp4"
FRAMES = 150
FPS = 30
...
# 表面记号（红点）配置
MARKER_LAT = 0       # 记号所在纬度（度，+ 为北半球）
MARKER_LON = 0       # 记号所在经度（度）
MARKER_RADIUS = 5    # 记号的角半径（度）
...
# 加载纹理，并在纹理图上按经纬度"涂"一个红点（随球面自转，无凸起）
from PIL import Image, ImageDraw
_tex_img = Image.open(TEXTURE_PATH).convert('RGB')
_W, _H = _tex_img.size
_cx = (MARKER_LON % 360) / 360 * _W          # 经度 -> 横向像素
_cy = (90 - MARKER_LAT) / 180 * _H           # 纬度 -> 纵向像素（v=0 在北极）
_rx = MARKER_RADIUS / 360 * _W / max(np.cos(np.radians(MARKER_LAT)), 0.05)  # 补偿经度随纬度的收缩
_ry = MARKER_RADIUS / 180 * _H
_draw = ImageDraw.Draw(_tex_img)
for _dx in (-_W, 0, _W):  # 横向周期环绕，保证跨接缝（u=0/1）的点完整
    _draw.ellipse([_cx + _dx - _rx, _cy - _ry, _cx + _dx + _rx, _cy + _ry], fill=(0, 134, 139))
tex = pv.numpy_to_texture(np.array(_tex_img))
