"""批量把 NSA、SA 和 NSA_anchor 纹理贴到球面，并生成六种旋转方向的视频。"""
...
OUTPUT_ROOT = PROJECT_ROOT / "videos"
ANCHOR_OUTPUT_ROOT = OUTPUT_ROOT / "NSA_anchor"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "批量为 textures/NSA、textures/SA、textures/NSA_anchor 中的 JPG/PNG "
            "生成六方向球体旋转视频"
        )
    )
    parser.add_argument(
...
def main() -> None:
    ...
    for category in args.categories:
        ...
        for image_path in images:
            for direction, (axis, sign) in ROTATIONS.items():
                output_root = (
                    ANCHOR_OUTPUT_ROOT if category == "NSA_ANCHOR" else OUTPUT_ROOT
                )
                output_path = (
    ...
    print(
        f"共发现 {len(jobs)} 个旋转视频任务；NSA/SA 输出到 {OUTPUT_ROOT}，"
        f"NSA_ANCHOR 输出到 {ANCHOR_OUTPUT_ROOT}"
    )
    generated = 0
    skipped = 0
    failures = []
